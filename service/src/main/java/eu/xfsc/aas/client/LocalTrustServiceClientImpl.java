    ...
    public Map<String, Object> evaluate(String policy, Map<String, Object> params) {
        ...
        if (GET_LOGIN_PROOF_RESULT.equals(policy) || GET_IAT_PROOF_RESULT.equals(policy)) {
            if (isPending(requestId)) {
                ...
                // SPDX-License-Identifier: Apache-2.0
                // SPDX-FileContributor: Modified by Software GmbH
                //MADI added for getting infos from OCM
                //AcapyClient ac = new AcapyClient();
                //String conid = ac.getAcapyConnection(requestId);
                //String pres =  ac.getPendingPresentationExchange(requestId,"verified");
                //log.debug("SHOW ConectionID: {}, SHOW Presentation :{}",conid,pres);
                //end
                // SPDX-SnippetEnd
